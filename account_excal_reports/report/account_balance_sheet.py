...
class Parser(report_sxw.rml_parse, common_report_header):
    ...
    def compute_currency(self,to_currency, from_currency, amt):
        ...
        amount = 0.0
        if to_currency:
            amount = currency_obj.compute(self.cr, self.uid, to_currency[0], from_currency, amt)
        return amount
    
    def get_symbol(self, currency_id):
        cur_id = currency_id[0]
        currency_obj = self.pool.get('res.currency')
        currency_rec = currency_obj.read(self.cr, self.uid, cur_id, ['symbol','name'])
        return_rec = ''
        if currency_rec.get('symbol', False):
            return_rec = currency_rec['symbol']
        else:
            return_rec = currency_rec['name']
        return return_rec
